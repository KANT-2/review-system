...
from .models import AIUsageLog, PRDProject, PRDQuestion, PRDSection, PostIt, PostItConnection
...
TEMPLATE_TYPES = [
    {
        ...
        "color": "indigo",
        "variants": [
            {"id": "b2c-app", "label": "B2C 앱/서비스", "description": "개인 사용자를 타겟으로 하는 소비자 서비스", "example": "학습 앱, 건강 관리, 커머스, 구독 서비스 등"},
            {"id": "b2b-saas", "label": "B2B SaaS/플랫폼", "description": "기업 또는 팀을 고객으로 하는 비즈니스 소프트웨어", "example": "협업 툴, 분석 플랫폼, ERP/CRM, API 서비스 등"},
            {"id": "internal-tool", "label": "사내 도구/어드민", "description": "내부 업무 효율화를 위한 운영 도구", "example": "고객 관리 어드민, 데이터 파이프라인, 모니터링 대시보드 등"},
        ],
    },
    {
        "id": "new-feature",
        ...
        "color": "violet",
        "variants": [
            {"id": "core-feature", "label": "핵심 기능 추가", "description": "서비스 핵심 가치를 확장하는 새로운 기능", "example": "소셜 기능, 분석 리포트, 자동화 기능 등"},
            {"id": "integration", "label": "외부 서비스 연동", "description": "타 플랫폼·API와의 통합 기능", "example": "Slack 알림, 캘린더 연동, 결제 게이트웨이 추가 등"},
            {"id": "channel-expand", "label": "채널/기기 확장", "description": "새로운 접점이나 플랫폼으로 서비스 확장", "example": "모바일 앱, 웹 클리퍼, 위젯, 이메일 다이제스트 등"},
        ],
    },
    {
        "id": "improvement",
        ...
        "color": "emerald",
        "variants": [
            {"id": "ux-improvement", "label": "UX/사용성 개선", "description": "사용자 경험과 인터페이스 개선", "example": "업로드 플로우 개선, 온보딩 최적화, 에러 안내 강화 등"},
            {"id": "performance", "label": "성능/기술 개선", "description": "속도, 안정성, 확장성 향상", "example": "검색 속도 개선, 캐시 최적화, 마이그레이션 등"},
            {"id": "policy-change", "label": "정책/비즈니스 규칙 변경", "description": "운영 정책이나 비즈니스 로직 변경", "example": "결제 정책 변경, 권한 구조 개편, 약관 업데이트 등"},
        ],
    },
]
...
DEFAULT_SECTIONS = [
    {
        "title": "프로젝트 요약", "step": 1,
        "guidance": "서비스를 처음 접하는 사람도 이해할 수 있도록 3~5줄로 설명하세요.",
        "questions": ["프로젝트명은 무엇인가요?", "한 줄 소개를 작성해주세요.", "해결하려는 핵심 문제는 무엇인가요?", "최초 출시 범위는 어디까지인가요?"],
    },
    {
        "title": "서비스 목표 및 핵심 가치", "step": 1,
        "guidance": "어떤 기능을 만들지보다, 사용자의 상황을 어떻게 바꿀지 작성하세요.",
        "questions": ["서비스 목표를 한 문장으로 정의해주세요.", "사용자가 얻는 핵심 가치는 무엇인가요?", "기존 대안 대비 차별점은?"],
    },
    {
        "title": "추진 배경 및 기회", "step": 2,
        "guidance": "왜 이 문제가 존재하고, 왜 지금 해결할 가치가 있는지 설명하세요.",
        "questions": ["문제를 발견한 계기는 무엇인가요?", "시장 동향 및 기회를 설명하세요.", "경쟁사·기존 대안의 한계는 무엇인가요?"],
    },
    {
        "title": "핵심 타겟 및 이용 상황", "step": 3,
        "guidance": "넓게 정의하지 마세요. 문제가 특히 자주 발생하는 첫 사용자군을 선정하세요.",
        "questions": ["최우선 타겟은 누구인가요?", "이 타겟을 먼저 선택한 이유는?", "대표 페르소나의 이용 상황을 구체적으로 설명하세요."],
    },
    {
        "title": "핵심 MVP 범위", "step": 4,
        "guidance": "서비스의 핵심 가치를 경험하는 데 필요한 최소 범위를 정하세요.",
        "questions": ["핵심 유저 플로우를 단계별로 작성해주세요.", "Must Have 기능 목록을 나열해주세요.", "Out of Scope로 명확히 제외할 기능은?"],
    },
    {
        "title": "성공 지표 및 검증 계획", "step": 6,
        "guidance": "출시했다는 사실이 아니라, 사용자가 핵심 가치를 얻었는지 판단할 지표를 작성하세요.",
        "questions": ["핵심 정량 지표(KPI)는 무엇인가요?", "정성적 성공 기준은 무엇인가요?", "목표 달성 시 후속 방향은?"],
    },
]
...
def home(request):
    ...
    soon_deadline = sum(
        1 for p in projects if p.deadline and 0 <= (p.deadline - today).days <= 7
    )
...
def prd_write(request, pk):
    project = get_object_or_404(
        PRDProject.objects.prefetch_related("sections__questions", "members"), pk=pk, members=request.user
    )
    ...
    members = [
        {**_serialize_member(m), "contribution_pct": 0} for m in project.members.all()
    ]
...
def ai_coach_ask(request, pk):
    ...
    try:
        reply = ask_ai_coach(
            project, message=message, current_section_title=current_section_title
        )
    except AICoachError:
        return JsonResponse(
            {"ok": False, "error": "AI Coach 응답을 가져오는 중 문제가 발생했습니다. 잠시 후 다시 시도해주세요."},
            status=502,
        )
