...
| ACC-004 | 필수 | 참가자는 중복 배정 없이 최대 한 팀에 속한다. | 중복 배정 참가자가 있으면 저장·시작이 거부된다. 미배정 참가자가 있어도 튜터가 확인하면 저장·시작할 수 있고, 그 참가자는 해당 회차 평가·점수 계산에서 빠진다. |
...
When 튜터가 확인 없이 회차 시작을 요청하면
Then 상태는 DRAFT로 유지되고 미배정 참가자를 안내한다

Given 회차 참가자 중 한 명이 팀에 배정되지 않았을 때
When 튜터가 미배정 참가자를 확인하고 회차 시작을 요청하면
Then 회차는 시작되고 그 참가자는 이번 회차 평가·점수 계산에서 빠진다
```
