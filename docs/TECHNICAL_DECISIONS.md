...
- **저장 척도는 5점 만점으로 통일하고 raw와 표시 정밀도를 분리한다.** 입력부터 팀/개인
  점수·최종점수·Seed까지 모두 1~5 척도로 저장한다. 계산 체인 전체는 raw 정밀도(소수
  6자리, `*_raw` 컬럼과 동일)를 유지하고, 화면에 보여줄 때만 `ROUND_HALF_UP`으로 소수
  둘째 자리로 반올림한다(`display_score`). 매 계산 단계마다 2자리로 반올림하면 오차가
  누적되므로 하지 않는다.
